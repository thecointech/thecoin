...
test('Verified signature matches', async () => {
	...
	const value = 100000;
	const fee = 2000;
	const verifiedXfer = await BuildVerifiedXfer(wallet, wallet.address, value, fee);
	
	const {timestamp, signature} = verifiedXfer
	// verify that this signature matches what the contract expects
	var signer = await contract.recoverSigner(wallet.address, wallet.address, value, fee, timestamp, signature);
