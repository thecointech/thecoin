...
from run_endpoint_query import MAX_RESOLUTION, Crop, get_image, position_to_pixels
...
async def process_image_query(image: UploadFile, prompt: str, json_description: str=None, crop_top: int=None, crop_height: int=None) -> ImageQueryResponse:
    try:
        ...
        if (json_description):
            ...
            return ImageQueryResponse(result=json.dumps(response))
        else:
            return ImageQueryResponse(result=response)
