...
class BillPaymentsClass extends React.PureComponent<Props, State> {
	...
	async processRawBill(bill: any, index: number)
	{
		...
		const decryptedPayee = DecryptPayee(this.state.privateKey, encryptedPayee);
	...
	{
		const { recievedTimestamp, processedTimestamp, encryptedPayee, transfer } = bill;
		...
		return (
			<Accordion>
				<Accordion.Title active={active} index={index} onClick={this.onBillAccordionClicked}>
					<Icon name='dropdown' />
					{encryptedPayee.name} - THE: {toHuman(transfer.value)}
				</Accordion.Title>
				<Accordion.Content active={active}>
					<div>CAD: ${cad}</div>
					<div>Recieved: {recievedAt.toLocaleTimeString()}</div>
					{processOn}
					<div>Payee: {decryptedPayee.payee}</div>
		...
		const bills = unsettledBills.map((bill, index) => {
			...
			return (
				<List.Item key={bill.hash}>
					<List.Content>
						<List.Header>{recDate.toDateString()} - {bill.encryptedPayee.name.substr(0, 10)}</List.Header>
