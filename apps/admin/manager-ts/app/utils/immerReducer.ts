...
function GetNamedReducer<T extends ImmerReducerClass>(immerReducerClass: T, name: string, initialState, dictionaryName?: string) 
  : { reducer: any; actions: ActionCreators<T>; reducerClass: T }
{
  if (!reducerCache[name]) {
    var actionCreators = undefined;
    
    class ExtendedReducer extends immerReducerClass {
      static customName = name
    };
    // We use Object.Assign to clone the prototype as we 
    // need to change the actions() function below and
    // need to ensure that change is local to the ExtendedReducer class
    Object.assign(ExtendedReducer.prototype, immerReducerClass.prototype);
    ...
    ExtendedReducer.prototype.actions = () => actionCreators;

    reducerCache[name] = {
      reducer, 
      actions: actionCreators,
      reducerClass: ExtendedReducer
    };
  }
...
function buildNamedDictionaryReducer(dictionaryFilter: string) 
{
  ...
  if (!existing) {
    ...
    existing = function(state: Dictionary<any>, action) 
    {
      let newState = state;
