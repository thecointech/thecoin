...
//   const data = getData();
//   const periodReturns = calcAllReturns(data, params);
...
//   const params = createParams({maxDuration: {years: 5}})
//   const periodReturns = calcAllResultsImmediate(slice, params);
...
test('Calculates average/min/max correctly', async () => {
  ...
  const averageReturns = calcAllResultsImmediate({data: data.slice(idx), params});
  ...
  expect(averaged10Yr.values[0].date.toSQLDate()).toBe('2010-06-26');
});
