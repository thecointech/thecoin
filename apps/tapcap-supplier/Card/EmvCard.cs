...
namespace TapCapSupplier.Card
{
	...
	public class EmvCard : IEmvCard, IDisposable
	{
		...
		/// </summary>
		public EmvCard(IWebHostEnvironment appEnv)
		{
			...
			{
				logger.Error("Error loading cache: {0}", e.Message);
			}
		...
		byte[] AcquireLockAndDo(Func<byte[]> func, bool initialize = true)
		{
			...
			lock (__CardLock)
			{
				...
				if (res == null)
				{
					logger.Info("Last tx failed tx: attempting retry");
					DoStaticInit(wasInitialized);
					res = func();
					if (res == null)
						logger.Error("Regenerating failed: please check the logs");
					else
						logger.Info("Regeneration successful, be on your way good sirs");
		...
		byte[] IEmvCard.GetSingleResponse(QueryWithHistory queryWithHistory)
		{
			...
			{
				logger.Error("Mismatched arrays");
				return null;
			}
		...
		byte[] CheckCachedResponse(byte[] query, byte[] clientResponse)
		{
			...
			if (cacheResponse == null)
			{
				...
				if (idx >= 0)
				{
					...
					logger.Info("Caching: [{0}] {1} => {2}", responses.ResponseParentIndex[idx], BitConverter.ToString(responses.Queries[idx]), BitConverter.ToString(responses.Responses[idx]));
				}
			}
			else
			{
		...
		private void DoStaticInit(int saveInitStatus)
		{
			...
			lock(__CardLock)
			{
				...
				{
					logger.Error("Error in StaticInit: {0}", err.ToString());
				}
			}
		}
