...
export class Wizard extends React.PureComponent<Props> {
  ...
  public onCancelSetState = () => this.setState({ selectedOption: null });
  ...
  public renderValueFormRadio = (
    value: number,
    current: ValueLimits | undefined,
  ) => (
    <Form.Radio
      label={value}
      value={value}
  ...
  public renderTechChoiceForm = () => (
    <Form.Group inline>
      <Form.Checkbox
        ...
        label="Installing Browser Extensions"
      />
      <Form.Checkbox
        name="installAppn"
        onChange={this.handleChangeCB}
        label="Installing a new Browser"
      />
    ...
    <>
      <Form.Group inline>
        {this.renderFormRadio(
          'Restrict access to my personal device',
          'accessible',
          'false',
        )}
        {this.renderFormRadio(
          'I want access from any device',
          'accessible',
          'true',
        )}
      </Form.Group>
  ...
  public renderRecommendationForm = () => {
    ...
    if (recommended.length > 0) {
      return recommended.map(rec => (
        ...
        <Form.Button key={rec.name} onClick={e => this.onSetStorage(e, rec)}>
          {rec.name}
        </Form.Button>
      ));
    }
    return (
      <p>
        <FormattedMessage {...messages.noRecommendRecommend} />
      </p>
    );
  };
  ...
  public render() {
    ...
    const actionDlg = this.state.selectedOption ? (
      <CreateOptionDlg
        onCancel={this.onCancelSetState}
