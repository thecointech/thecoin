...
function printWallet(printWindow: Window|null, accountName: string)
{
  ...
  QRCode.toCanvas(canvas, accountString, function (error) {
      ...
      <List celled verticalAlign='middle'>
        <List.Item>
          <List.Content floated='right'>
            <Button onClick={() => {
              var printWindow = window.open("", props.accountName, "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=780,top="+(screen.height-200)+",left="+(screen.width-200));
        ...
        <List.Item>
          <List.Content floated='right'>
            <Download accountName={props.accountName} onComplete={() => setDone(true)} />
          </List.Content>
          <Icon name="download" />
          <List.Content>
