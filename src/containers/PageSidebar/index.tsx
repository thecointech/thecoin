...
class PageSidebar extends React.PureComponent<Props, {}, null> {
  ...
  buildMenuArray(items: SidebarMenuItem[]): React.ReactChild[] {
    return items.map(item => (item.link.to !== false ? this.getAsItem(item) : this.getAsDivider(item)));
  }

  buildSubMenuArray(item: SidebarMenuItem) {
    const { subItems } = item;
    return subItems ? <Menu.Menu>{this.buildMenuArray(subItems)}</Menu.Menu> : undefined;
  }
  ...
  render() {
    ...
    return (
      <Sidebar.Pushable as={Segment} className={styles.mainPageContainer}>
        <Sidebar as={Menu} animation="push" direction="left" vertical visible={visible} className={styles.mainPageSidebar} inverted={inverted}>
          {menuItems}
        </Sidebar>
        <Sidebar.Pusher className={pusherClass}>{this.props.children}</Sidebar.Pusher>
      </Sidebar.Pushable>
    );
  }
}
...
const ConnectedPageSidebar = buildReducer<OwnProps>()(connect(mapStateToProps)(PageSidebar));
