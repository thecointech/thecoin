...
const step: MigrationStep = () =>
  async (deployer, network, _accounts) => {
    ...
    const mintTestTokens = network == 'polygon' && (config === 'devlive' || config === 'prodtest');
    if (!mintTestTokens)
      return;
    ...
    if (config === 'devlive') {
      console.log("Transfering test token to client1");
    ...
    const tokenFile = join(__dirname, `../${config}-${network}.tokens.json`);
    console.log(`Wrote token keys to : ${tokenFile}`);
