...
export const getProvider = () => {
  ...
  if (!process.env.DEPLOY_POLYGON_NETWORK)
  ...
  if (process.env.NODE_ENV === "production" && !process.env.INFURA_PROJECT_ID)
    throw new Error(`Missing INFURA project ID, cannot connect to ${process.env.DEPLOY_POLYGON_NETWORK}`);

  return (process.env.INFURA_PROJECT_ID)
    ? new providers.InfuraProvider(process.env.DEPLOY_POLYGON_NETWORK, process.env.INFURA_PROJECT_ID)
