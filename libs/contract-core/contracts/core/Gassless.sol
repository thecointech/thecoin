...
abstract contract Gassless is Mintable {
    ...
    timestampIncreases(from, timestamp)
  {
	...
	function buildMessage(address from, address to, uint256 value, uint256 fee, uint256 timestamp) public pure returns (bytes32)
	{
		bytes memory packed = abi.encodePacked(from, to, value, fee, timestamp);
		return keccak256(packed);
	}

	function recoverSigner(address from, address to, uint256 value, uint256 fee, uint256 timestamp, bytes memory signature) public pure returns (address)
	{
